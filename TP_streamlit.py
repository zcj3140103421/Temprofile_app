...
def compute_temperature(k_list, thickness_list, T_bottom, q_flux, num_nodes=1000):
    ...
    for i, pos in enumerate(z):
        for layer_idx in range(len(k_list)):
...
# ==================== 绘制结构示意图（支持任意层数） ====================
def draw_schematic(num_layers, use_groove, depth_ratio=0.4, figsize=(4, 3)):
    ...
    for i in range(num_layers):
        ...
        if i == 0 and use_groove:
            ...
            for j in range(num_grooves):
                xg0 = start_x + j * (groove_width + spacing)
            ...
            ax.text((x0+x1)/2, (y_top + y_bottom)/2 + 0.3, f'层{i+1}', ha='center', va='center',
...
if st.sidebar.button("🚀 计算", type="primary"):
    try:
        ...
        # 动态生成界面名称
        names = ["抛光表面 (顶部)"]
        for i in range(len(k_eff_list)-1):
            names.append(f"层{i+1}/{i+2}界面")
        names.append("副垫层底部 (压板)")
        ...
        with right_col:
            st.subheader("📐 结构示意图")
            fig_schem = draw_schematic(num_layers, use_groove, depth_ratio if use_groove else 0.4, figsize=(4.5, 3.5))
            st.pyplot(fig_schem)
